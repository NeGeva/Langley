...
do
	...
	local slamtime = 0
	local now
	local slam = GetSpellInfo(1464)--ÃÍ»÷
...
local function Ranged(self, event, unit, spellName)--ÅÐ¶ÏÉä»÷
	if unit ~= "player" then return end
	if spellName ~= GetSpellInfo(75) and spellName ~= GetSpellInfo(5019) then return end--75×Ô¶¯Éä»÷£¬5019Éä»÷
...
local function Melee(self, event, _, subevent, _, GUID)--ÅÐ¶Ï¹¥»÷
...
--> Creat SwingBae
--- ----------------------------------
function createSwingBar(self)
	--StatusBar
	self.Swing = CreateFrame("Frame", nil, UIParent)
	self.Swing.texture = ns.Tex.Bar.Bar
	--self.Swing:GetStatusBarTexture():SetHorizTile(false)
	self.Swing.color = {95/255, 182/255, 255/255 ,1}
	--self.Swing:SetFrameLevel(2)
	self.Swing:SetPoint("TOPRIGHT", self.Castbar, "BOTTOMRIGHT", 0, -4)
	self.Swing:SetHeight(10)
	self.Swing:SetWidth(250+20)
end
